...
def login_submit(request: Request, username: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    resp.set_cookie("turnos_pending", pending, httponly=True, samesite="lax", max_age=settings.PENDING_MFA_EXPIRE_MINUTES * 60)
    return resp


@router.get("/mfa/setup", response_class=HTMLResponse)
def mfa_setup_form(request: Request, pending: str | None = Cookie(default=None, alias="turnos_pending"), db: Session = Depends(get_db)):
...
def mfa_setup_submit(
    request: Request, code: str = Form(...), pending: str | None = Cookie(default=None, alias="turnos_pending"), db: Session = Depends(get_db)
):
    user = _pending_user(pending, db)
    if not user or user.mfa_enabled:
        return RedirectResponse("/login", status_code=303)
    ...
    resp.delete_cookie("turnos_pending")
    resp.set_cookie("turnos_session", session, httponly=True, samesite="lax", max_age=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60)
    return resp


@router.get("/mfa/verify", response_class=HTMLResponse)
def mfa_verify_form(request: Request, pending: str | None = Cookie(default=None, alias="turnos_pending"), db: Session = Depends(get_db)):
    user = _pending_user(pending, db)
...
def mfa_show(request: Request, pending: str | None = Cookie(default=None, alias="turnos_pending"), db: Session = Depends(get_db)):
...
def logout():
    ...
    resp.delete_cookie("turnos_session")
    return resp
